  ...
  const client = {
    from(table: string) {
      ...
      const chain = {
        ...
        async single() { return { data: table === "accounts" ? { metadata: { outreachEligibility: "ELIGIBLE", prospectIntelligence: { outreachEligibility: "ELIGIBLE", salesMotion: "DIRECT", nextBestCommercialAction: { type: "PRODUCT_EXPLORATION", productDestinationUrl: "https://www.eventsuite.pro/", resourceOffer: { canonicalUrl: "https://www.eventsuite.pro/resources" } } } } } : { status: "ACTIVE" }, error: null }; },
