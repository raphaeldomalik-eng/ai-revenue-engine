...
test("prospect list uses the compact drawer IA and has no sending control", async () => {
  ...
  assert.match(active, /setProspectTypeFilter\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /setTerritory\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /setReviewState\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /setContactFilter\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /setEmailFilter\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /setPriority\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /setQuery\(event\.target\.value\); resetPagination\(\)/);
  assert.match(active, /if \(queue !== key\).*setQueue\(key\); resetPagination\(\)/);
