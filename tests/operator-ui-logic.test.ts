...
import { contactShortLabel, contextLabel, discoverySignal, latestRun, needsReview, organisationName, resolutionLabel, runCounts, siteTypeLabel, sourceType, type OperatorCandidate, type OperatorRun } from "../src/operator-ui/logic.ts";
...
test("collection display helpers preserve commercial meaning and conservative contactability", () => {
  assert.equal(organisationName(candidate({ organiser_name: "Resolved Organisation" })), "Resolved Organisation");
  assert.equal(organisationName(candidate({ organiser_name: null, account: null })), "ORGANISATION NOT YET RESOLVED");
  assert.equal(discoverySignal(candidate()), "Festival X");
  assert.equal(resolutionLabel(candidate({ prospect_intelligence: { organisationResolution: { status: "RESOLVED", confidence: "HIGH" } } })), "Resolved · HIGH");
  assert.equal(resolutionLabel(candidate({ prospect_intelligence: { organisationResolution: { status: "UNRESOLVED" } } })), "Unresolved");
  assert.equal(contactShortLabel(candidate()), "No verified target contact");
});
