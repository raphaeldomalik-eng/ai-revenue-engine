-- Incoming submissions may create an activity and a review queue entry, but never
-- a product opportunity until an operator has explicitly qualified the lead.
...
declare
  ...
  v_key text; v_activity_count integer; v_existing_category text; v_existing_stage text; v_product_name text; v_summary text; v_org_confidence text; v_lead_classification text := 'NEEDS_REVIEW';
begin
  ...
  if v_submission_id is null then
    ...
    select incoming_lead_id into v_lead_id from public.incoming_submissions where id=v_submission_id;
  ...
  select id,activity_count,highest_intent_rank,highest_intent_source_category,stage,lead_classification into v_lead_id,v_activity_count,v_rank,v_existing_category,v_existing_stage,v_lead_classification from public.incoming_leads where canonical_key=v_key limit 1;
  ...
  -- New records begin NEEDS_REVIEW. Existing non-leads, customers, and excluded
  -- records remain ineligible even when a later high-intent submission arrives.
  if v_category in ('DEMO_REQUEST','TALK_TO_SALES','TRIAL_STARTED','PRODUCT_ENQUIRY') and v_account_id is not null and v_identity_state='RESOLVED' and v_lead_classification='GENUINE_PROSPECT' then
...
revoke all on function public.ingest_incoming_submission(jsonb) from anon;
