...
export async function POST(request: Request) {
  ...
  try {
    if (body.action === "prepare") {
      ...
      if (accountState?.metadata?.outreachEligibility !== "ELIGIBLE" || prospectIntelligence?.outreachEligibility !== "ELIGIBLE" || prospectIntelligence?.salesMotion !== "DIRECT") throw new Error(accountState?.metadata?.outreachEligibilityReason || "OUTREACH_REVIEW_REQUIRED");
