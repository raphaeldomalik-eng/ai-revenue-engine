  ...
  return <section className="lead-intelligence-section" aria-label="AI Sales Team">
    ...
    {briefs.map((brief: any, index) => {
      ...
      return <article className="intelligence-card" key={`${brief.account_id}-${brief.created_at}-${index}`}>
        ...
        {action ? <p className="card-meta">NEXT BEST COMMERCIAL ACTION · {action.rationale} {action.targetUrlIfVerified ? <a href={action.targetUrlIfVerified} target="_blank" rel="noreferrer">Verified route</a> : null} · Call now: {action.callRecommended ? "Yes" : "No"}</p> : null}
