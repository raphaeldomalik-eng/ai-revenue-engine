...
import { canSendMessage, knownRecipient } from "../ai-sales-team/outreach-model.ts";
...
export async function sendApprovedOutreachMessage(client: SupabaseClient, messageId: string, actorId: string) {
  ...
  if (!canSendMessage(message, sequence.status, false, false)) throw new Error(message.status !== "APPROVED" ? "OUTREACH_APPROVAL_REQUIRED" : "OUTREACH_RECIPIENT_UNKNOWN");
  ...
  try {
    const result = await sendEmail({ messageId, recipientEmail: recipient, subject: claimed.subject, body: claimed.body });
    await client.from("outreach_messages").update({ status: "SENT", provider: result.provider, provider_message_id: result.providerMessageId, sent_at: new Date().toISOString(), sent_subject: claimed.subject, sent_body: claimed.body, updated_at: new Date().toISOString() }).eq("id", messageId).eq("status", "SENDING");
