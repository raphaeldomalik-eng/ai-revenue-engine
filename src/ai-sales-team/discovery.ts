...
export async function enrichDiscoveryCandidates(candidates: EvaluatedDiscoveryCandidate[], territory: DiscoveryTerritory) {
  const targets = candidates.filter((candidate) => candidate.status === "REVIEW_REQUIRED" && candidate.relationship === "PROSPECT" && candidate.prospectIntelligence.eventConnection.state !== "NONE").slice(0, 8);
...
export async function discoverProspects(input: { territory: DiscoveryTerritory; focus: DiscoveryFocus }) {
  ...
  const candidates = await enrichDiscoveryCandidates(initial, input.territory);
