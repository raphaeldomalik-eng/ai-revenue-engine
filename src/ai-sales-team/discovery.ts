...
export async function discoverProspects(input: { territory: DiscoveryTerritory; focus: DiscoveryFocus }) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, tools: [{ type: "web_search" }], max_output_tokens: 12000, input: `${DISCOVERY_SCOUT_PROMPT_V1}\nFind up to eight current public EventSuite activity signals in ${territory}, focused on ${focus}. Today is ${new Date().toISOString().slice(0, 10)}. Return diverse evidence-backed signals and do not assign the discovery source as the commercial website.`, text: { format: { type: "json_schema", name: "prospecting_quality_foundation", strict: true, schema: candidateSchema } } } ) });
