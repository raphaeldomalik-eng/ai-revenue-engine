...
export type ComposerReviewAction = "APPROVE" | "REJECT" | "REQUEST_REVISION" | "EDIT_APPROVE" | "RATE";
...
export async function createComposerSequence(client: SupabaseClient, input: ComposerInput, actorId: string, accountId: string, briefId: string | null, contactId: string | null, fetchImpl?: typeof fetch) {
...
export async function reviseComposerDraft(client: SupabaseClient, input: ComposerInput, draftId: string, actorId: string, latestRevisionNumber: number, fetchImpl?: typeof fetch) {
  const generated = await generateOutreachComposerDraft(input, fetchImpl);
...
export async function recordComposerReview(client: SupabaseClient, input: { versionId: string; action: ComposerReviewAction; actorId: string; editedSubject?: string; editedBody?: string; relevanceRating?: number; toneRating?: number; reasonTags?: string[]; note?: string; stage?: "EMAIL_1" | "EMAIL_2" | "EMAIL_3" }) {
  ...
  if (!( ["APPROVE", "REJECT", "REQUEST_REVISION", "EDIT_APPROVE", "RATE"] as readonly string[]).includes(input.action)) throw new Error("OUTREACH_COMPOSER_REVIEW_INVALID: action");
