...
type Usage = { input_tokens?: number; input_tokens_details?: { cached_tokens?: number }; output_tokens?: number; output_tokens_details?: { reasoning_tokens?: number } };
...
function qualitySignals(frozen: FrozenCase, candidate: EvaluatedDiscoveryCandidate | null, contact: unknown) {
  const resolution = candidate?.organisationResolution;
  const contactSummary = compactContact(contact) as { blocked?: boolean; namedContactFound?: boolean; organisationRouteFound?: boolean } | null;
  return { lanePreserved: candidate?.origin === frozen.lane, credibleEntity: Boolean(candidate), authoritativeSourceCount: candidate?.facts.filter((item) => Boolean(item.sourceUrl)).length ?? 0, identityResolved: resolution?.status === "RESOLVED", relationshipSafe: candidate ? candidate.relationship !== "COMPETITOR" && candidate.status !== "BLOCKED" : true, safeUnresolved: Boolean(candidate && resolution?.status === "UNRESOLVED"), commercialAdvanced: Boolean(candidate?.enrichment.commerciallyAdvanced), namedBuyerFound: Boolean(contactSummary?.namedContactFound), legitimateContactRoute: Boolean(contactSummary?.organisationRouteFound), contactEligibleOrBlocked: Boolean(contactSummary?.blocked) || Boolean(contactSummary) };
}
...
export async function runLiveModelComparison() {
  ...
  try {
    for (const config of [{ model: "gpt-4.1-mini", reasoningEffort: "none" }, { model: "gpt-5.6-terra", reasoningEffort: "medium" }] as const) {
  ...
  const usage = calls.map((call) => { const u = call.usage ?? {}; const input = u.input_tokens ?? 0; const cached = u.input_tokens_details?.cached_tokens ?? 0; const output = u.output_tokens ?? 0; const searchContent = call.model === "gpt-4.1-mini" ? call.webSearchCalls * 8000 : null; const price = PRICES[call.model as keyof typeof PRICES]; const uncachedInputCost = ((input - cached) * price.input) / 1_000_000; const cachedInputCost = (cached * price.cached) / 1_000_000; const outputCost = (output * price.output) / 1_000_000; const searchCost = searchContent === null ? null : (searchContent * price.input) / 1_000_000; return { ...call, inputTokens: input, cachedInputTokens: cached, outputTokens: output, reasoningTokens: u.output_tokens_details?.reasoning_tokens ?? 0, searchContentTokens: searchContent, uncachedInputCostUsd: uncachedInputCost, cachedInputCostUsd: cachedInputCost, outputCostUsd: outputCost, searchContentCostUsd: searchCost, totalCostUsd: uncachedInputCost + cachedInputCost + outputCost + (searchCost ?? 0) }; });
