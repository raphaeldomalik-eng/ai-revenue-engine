...
export async function runApolloSmokeTest() {
  ...
  const result = search as { results: Array<{ fullName: string | null; title: string | null; status: string; roleClassification: string | null; organisationDomain: string | null; rejectionReason: string | null }>; telemetry: unknown };
  return { smokeVersion: "apollo-v1", mode, caseId: frozen.id, health: { ok: true }, usage: { available: true }, search: { resultCount: result.results.length, people: result.results.map((person) => ({ fullName: person.fullName, title: person.title, status: person.status, roleClassification: person.roleClassification, domainAlignment: person.organisationDomain === "mashmedia.co.uk", rejectionReason: person.rejectionReason })), telemetry: result.telemetry }, noPeopleMatchCalled: true, noEmailOrPhoneRequested: true, noPersistenceOrOutreach: true };
}
