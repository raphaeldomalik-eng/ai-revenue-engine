...
async function runCase(item: LiveCase, originalFetch: typeof fetch, projectedCost: { value: number }) {
  ...
  const result: Record<string, unknown> = { id: item.id, lane: item.lane, signal: item.startingSignal, phaseOne: { classification: "PHASE_ONE_PRIORITY", reason: "Frozen official-source evidence supplies a documented Phase One priority signal; this is routing, not SME proof." }, lanePreserved: true, providerSequence, openAi: { calls: 0, model: MODEL, usage: null, webSearchCalls: 0 }, companiesHouse: { search: 0, profile: 0, outcome: null, reason: null, company: null }, googlePlaces: null, apollo: null, identities: { event: item.eventName, venue: item.venueName, operator: null, organisation: null, legalCompany: null }, conflicts: [], contactResearchAttempted: false, enrichmentAttempted: false, persistenceWrites: 0, outreachActions: 0 };
